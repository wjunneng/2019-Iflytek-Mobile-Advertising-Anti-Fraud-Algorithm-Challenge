...
class TrainModels():
    ...
    def _judge_catboost(self, train, test, features):
        model = CatBoostClassifier(iterations=946, depth=8, cat_features=features, learning_rate=0.05,
...
if __name__ == "__main__":
    ...
    train = pd.read_table(project_path + '/data/original/round1_iflyad_anticheat_traindata.txt')
    test = pd.read_table(project_path + '/data/original/round1_iflyad_anticheat_testdata_feature.txt')
    ...
    # judge_by_blackList.to_csv(project_path + '/data/submit/judge_by_blackList.csv', index=False, encoding='utf-8')
    # 二分类---使用catboost
    ...
    judge_by_catboost.to_csv(project_path + '/data/submit/judge_by_catboost.csv', index=False, encoding='utf-8')
